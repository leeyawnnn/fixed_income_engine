...
reruns it on every push and fails if any of these values moves — compared
numerically, to a tolerance far tighter than the precision quoted here, because
Apple's and GNU's math libraries disagree in the sixteenth significant figure
and a byte-exact check would fail on that alone.
...
CSVs in `reports/`. On one machine the output is byte-identical between runs;
across machines the embedded values differ in their last digit, so CI checks
that every figure still renders rather than that the bytes match.
